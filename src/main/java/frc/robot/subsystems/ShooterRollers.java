...
public class ShooterRollers extends SubsystemBase {
  ...
  public ShooterRollers() {
    ...
    shooterConfig.MotorOutput.PeakForwardDutyCycle = 0.9;
    shooterConfig.MotorOutput.PeakReverseDutyCycle = -0.9;
  ...
  double negative;
  public void wheelsShooter(double speed) {
    ...
    /*if (speed < 0) {
     ...
     if (speed < 0) {
       m_shooterMotor.set(Math.min(speed, 0));
     } else {
       m_shooterMotor.set(getPercentFromBattery(speed));
     }
  ...
  public void periodic() {
    SmartDashboard.putNumber("ShooterRollerCurrent", m_shooterMotor.getStatorCurrent().getValueAsDouble()); 
    SmartDashboard.putBoolean("RollersHappy", m_shooterMotor.getStatorCurrent().getValueAsDouble()<25); 
